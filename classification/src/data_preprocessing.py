...
def get_preprocessing(directory):
    """
    This function augments the data in the given directory.
    It first creates the necessary directories for saving the augmented images.
    Then, it continuously applies preprocessing and saves the images until it
    reaches the 'max_images' limit for each class.
    ...
    datasets = tf.keras.utils.image_dataset_from_directory(
        ...
        labels='inferred',  # Classes will be inferred from the subdirectory
        label_mode='categorical',  # Labels will be one-hot encoded
        class_names=list,  # List of class names
        batch_size=16,  # Number of processed samples before updating weights
        ...
        crop_to_aspect_ratio=True  # Cropping the image to the aspect ratio
    )
